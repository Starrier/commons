...
public class TokenUtils {
     ...
     * @return
     */
    public static String generateToken(Map<String, Object> claims) throws NoSuchAlgorithmException, InvalidKeyException, InvalidKeySpecException {
        return Jwts.builder()
                .setClaims(claims)
     ...
     * @return
     */
    private static Optional<Claims> getClaimsFromToken(String token) throws NoSuchAlgorithmException, InvalidKeyException, InvalidKeySpecException {
     ...
     * @return
     */
    private static Map<String, Object> extractInfo(String token) throws NoSuchAlgorithmException, InvalidKeyException, InvalidKeySpecException {
        Optional<Claims> claims = getClaimsFromToken(token);
        if (claims.isPresent()) {
            Set<String> keySet = claims.get().keySet();
            Map<String, Object> info = Maps.newHashMapWithExpectedSize(keySet.size());
            keySet.forEach((String key) -> info.put(key, claims.get().get(key)));
            return info;
        }
